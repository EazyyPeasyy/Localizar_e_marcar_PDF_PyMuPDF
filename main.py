...
# ### Read list of words to highlight from file "query.txt" in same folder
# with open('query.txt') as f:
#     queries = f.readlines()

# ### List PDF files in folder
# files = [f for f in os.listdir('.') if os.path.isfile(f) and f.endswith('.pdf')] # creates a list of pdf files in folder

# ### Annotated PDF name modifier
# mod = 'highlighted'
# i = 0

# ### HIGHLIGHT
# for file in files: # for each PDF file in the folder
#     doc = fitz.open(file) # open the PDF
#     for page in doc:  # for each page in the PDF
#         for query in queries: # for each sentence to be highlighted
#             text_instances = page.searchFor(query) # search for the sentence in the page
#             for inst in text_instances: # for each sentence found in page
#                 highlight = page.addHighlightAnnot(inst) # highlight the sentence
#                 i+=1
#     doc.save(mod+'-'+file(str(i)+' sentences were highlighted in '+str(len(files))+' files')


#    SITE QUE EXPLICA COMO FAZER MELHOR 

#     https://morioh.com/p/5b1876ffa72e
